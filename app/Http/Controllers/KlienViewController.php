...
class KlienViewController extends Controller
{
    public function viewKlien()
    {
        ...
        $data = KlienView::joinSub($subQuery, 'latest', function ($join) {
                $join->on('klien_view.id_pk', '=', 'latest.id_pk')
                    ->on('klien_view.tkh_tamatPengawasan', '=', 'latest.latest_tkh_tamatPengawasan');
            })
            ->whereIn('klien_view.id_fasiliti', ['16', '31', '45', '57', '69', '80', '90', '99', '106'])
            ->limit(1000) 
            ->get()
            ->toArray();

...
class PelaporanController extends Controller
{
    ...
    public function PDFAnalisisMK(Request $request) 
    {
        ...
        // Generate PDF
        $pdf = PDF::loadView('modal_kepulihan.pentadbir_pegawai.pdf_analisis_modal_kepulihan', compact('counts', 'modalKepulihan', 'totalClients'))
    ...
    public function jsonPDFtidakPernahMenjawabPB(Request $request) 
    {
        ...
        $filteredData = $query->limit(5000)->get(); // Limit records to prevent overload

        return response()->json([
            'success' => count($filteredData) > 0,
            'data' => $filteredData
        ]);
    }
